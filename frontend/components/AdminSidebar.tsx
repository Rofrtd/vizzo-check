...
function agenciesMenuItem(basePath: string): MenuItem {
  return {
    title: 'Agências',
...
export default function AdminSidebar() {
  ...
  const { basePath, panelLabel } = usePanel();
  ...
  const menuItems: MenuItem[] = useMemo(() => [
    ...(isSystemAdmin ? [agenciesMenuItem(basePath)] : []),
    {
      title: 'Dashboard',
  ...
  ], [basePath, isSystemAdmin]);

  const isActive = (href: string) => {
    if (href === `${basePath}/dashboard`) {
      return pathname === href;
    }
    if (href === `${basePath}/agencies`) {
      return pathname === href;
    }
        ...
        <div className="p-6 border-b border-gray-700/50 relative overflow-hidden">
          ...
          <div className="relative flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 font-medium">{panelLabel}</p>
            </div>
          </div>
        </div>
